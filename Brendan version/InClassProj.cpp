...
//the following line uses Player as though it inherits from the Sprite class and will not work with Rhyse's new code
	...
	std::vector<Sprite::Frame*> homeTile;
	homeTile.push_back(mTiles[0]);
	...
	board[125][125] = new Tile(XMVectorSet(250.0f, 500.0f, 0.0f, 0.0f), XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f), 250, 250, 0.0f, homeTile, 1.0f, md3dDevice);
	//TilePos = XMVectorSet(((curCol - 125) * 250), ((curRow - 125) * 250), 0.0f, 0.0f);

	/*std::vector<Sprite::Frame*> tile;
	tile.push_back(mTiles[1]);
	TileToBePlaced = new Tile(GetTilePos(), XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f), 250, 250, 0.0f, tile, 1.0f, md3dDevice);
	*/
...
void InClassProj::UpdateScene(float dt)
{
	...
	TileToBePlaced = new Tile(XMVectorSet((curCol-125)*250, (curRow-125)*250, 0.0f, 0.0f), XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f), 250, 250, 0.0f, tile2, 1.0f, md3dDevice);
...
XMVECTOR InClassProj::GetTilePos() const
{
	return TilePos;
}
